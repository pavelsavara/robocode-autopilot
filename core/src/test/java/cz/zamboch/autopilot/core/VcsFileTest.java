...
final class VcsFileTest {
    ...
    void loadFromNonExistentFileReturnsEmptyStore() {
        ...
        assertNotNull(store);
        assertEquals(GuessFactor.ZERO_BIN, store.getBestBin(0, 0, 0.0));
    }
    ...
    void saveAndLoadSingleOpponent() throws IOException {
        ...
        store.increment(0, 0, 0.0, 20);
        store.increment(0, 0, 0.0, 20);
        VcsFile.saveForOpponent(f, hash, store);

        VcsStore loaded = VcsFile.loadForOpponent(f, hash);
        assertEquals(2, loaded.getCount(0, 0, 0.0, 20));
        assertEquals(20, loaded.getBestBin(0, 0, 0.0));
    }
    ...
    void upsertExistingOpponent() throws IOException {
        ...
        VcsStore store = new VcsStore();
        store.increment(0, 0, 0.0, 10);
        VcsFile.saveForOpponent(f, hash, store);

        // Update
        store.increment(0, 0, 0.0, 10);
        store.increment(2, 2, 0.0, 30);
        VcsFile.saveForOpponent(f, hash, store);

        VcsStore loaded = VcsFile.loadForOpponent(f, hash);
        assertEquals(2, loaded.getCount(0, 0, 0.0, 10));
        assertEquals(1, loaded.getCount(2, 2, 0.0, 30));
    }
    ...
    void evictsOldestWhenOverSizeLimit() throws IOException {
        ...
        for (int i = 0; i < maxEntries + 5; i++) {
            ...
            s.increment(0, 0, 0.0, i % GuessFactor.NUM_BINS);
        ...
        assertEquals(1, last.getCount(0, 0, 0.0, lastHash % GuessFactor.NUM_BINS));

        // First opponent should have been evicted
        VcsStore first = VcsFile.loadForOpponent(f, 0);
        assertEquals(GuessFactor.ZERO_BIN, first.getBestBin(0, 0, 0.0));
    }
}
