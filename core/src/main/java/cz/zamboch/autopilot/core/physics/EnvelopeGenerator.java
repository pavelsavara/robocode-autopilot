...
public final class EnvelopeGenerator {
     ...
     * Output format matches the Base64 distillation style used by
     * FirePowerData / MovementData / FireTimingData / DefaultDataFile:
     * a single packed buffer (per velocity: u16 big-endian length prefix
     * followed by raw signed bytes), Base64-encoded, split into D0..Dn
     * string constants of at most {@value #B64_CHUNK_CHARS} characters
     * each to stay under the JVM's 64 KB-per-string-constant limit.
     *
     * Usage: java EnvelopeGenerator [output-dir]
     */
    private static final int B64_CHUNK_CHARS = 60_000;
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Pack: for each velocity, u16 big-endian length, then raw bytes.
        ...
        for (int v = 0; v <= 8; v++) {
            ...
            if (n > 0xFFFF) {
                throw new IllegalStateException(
                        "v=" + v + ": " + n + " bytes does not fit in u16 length prefix");
            }
            baos.write((n >>> 8) & 0xFF);
            baos.write(n & 0xFF);
            for (int[] p : points) {
        ...
        for (int i = 0; i < b64.length(); i += B64_CHUNK_CHARS) {
            parts.add(b64.substring(i, Math.min(b64.length(), i + B64_CHUNK_CHARS)));
        }

        StringBuilder summary = new StringBuilder();
        for (int v = 0; v <= 8; v++) {
            if (v > 0) summary.append(", ");
            summary.append("v").append(v).append("=").append(tables[v].size());
        }
        ...
        pw.println(" * Each entry is interleaved (dx, dy) signed-byte pairs relative to");
        pw.println(" * heading=0 (north). Indexed by |velocity| 0..8.");
        pw.println(" * Values fit in signed byte (-128..127); max offset is ~80px.");
        pw.println(" *");
        pw.println(" * <p>Position counts: " + summary + ".");
        pw.println(" * Total raw payload: " + raw.length + " bytes.");
        pw.println(" *");
        pw.println(" * <p>Data is Base64-encoded and decoded once at class load. On-disk");
        pw.println(" * format: for each velocity 0..8 a 2-byte big-endian length prefix is");
        pw.println(" * followed by the raw signed bytes. The Base64 string is split into");
        pw.println(" * chunks to stay under the JVM's 64\u00a0KB-per-string-constant limit.");
        pw.println(" */");
        pw.println("final class EnvelopeData {");
        pw.println("    private EnvelopeData() {}");
        pw.println();
        for (int i = 0; i < parts.size(); i++) {
            pw.println("    private static final String D" + i + " = \"" + parts.get(i) + "\";");
        }
        pw.println();
        pw.println("    /** Decoded tables, indexed by |velocity| 0..8. */");
        pw.println("    static final byte[][] ALL = decode();");
        pw.println();
        pw.println("    private static byte[][] decode() {");
        pw.println("        StringBuilder sb = new StringBuilder(" + b64.length() + ");");
        for (int i = 0; i < parts.size(); i++) {
            pw.println("        sb.append(D" + i + ");");
        }
        pw.println("        byte[] raw = java.util.Base64.getDecoder().decode(sb.toString());");
        pw.println("        byte[][] all = new byte[9][];");
        pw.println("        int off = 0;");
        pw.println("        for (int v = 0; v < 9; v++) {");
        pw.println("            int n = ((raw[off] & 0xFF) << 8) | (raw[off + 1] & 0xFF);");
        pw.println("            off += 2;");
        pw.println("            byte[] data = new byte[n];");
        pw.println("            System.arraycopy(raw, off, data, 0, n);");
        pw.println("            off += n;");
        pw.println("            all[v] = data;");
        pw.println("        }");
        pw.println("        return all;");
        pw.println("    }");
        pw.println("}");
        pw.close();
        System.err.println("Wrote " + outDir + "/EnvelopeData.java"
                + " (" + raw.length + " raw bytes, " + b64.length() + " base64 chars in "
                + parts.size() + " string(s))");
    }
}
